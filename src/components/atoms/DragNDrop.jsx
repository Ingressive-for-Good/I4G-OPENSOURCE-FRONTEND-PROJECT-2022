...
export default function DragNDrop() {
  ...
  return (
    <section className='relative'>
      {!files.length > 0 && <div  {...getRootProps(
      ...
      {files.length > 0 && <div className="flex flex-grow flex-wrap gap-[5px]"> 
        {
            files.map((file)=> (
                <div className="image-preview m-2 w-[140px] h-40 rounded-[8px] relative" key={file.name}>
                    ...
                    />
                    <img className="absolute top-[10px] right-[10px] cursor-pointer" src={DeleteIcon} alt="delete-icon" />
                </div>
            ))
        }
        <div  {...getRootProps(
          {className:'image-preview m-2  w-[140px] h-40 bg-neutral-500 rounded-[8px] flex justify-center items-center cursor-pointer'}
          )}>
